...
先运行 `codex plugin marketplace list --json` 和 `codex plugin list --json`，确认已有来源。已经从 GitHub 安装的，刷新同一个 marketplace；不要重新添加本地解压目录。网站版本和 GitHub 版本暂时不同时，报告实际版本，保留来源，不自动改装 ZIP。

不使用脚本时，可在当前 Codex CLI 中执行这两步：
...
脚本还会核对安装来源与稳定分支。当前 Codex 有的版本不在 marketplace 列表中显示分支；脚本会继续检查 Codex 保存的安装记录或实际 Git 分支，不把“没显示”误判成“装错了”。

Claude Code CLI 的 GitHub 来源使用 `claude plugin marketplace update open-study`，再运行 `claude plugin update open-study@open-study --scope user`。Claude Code 桌面端按自己的插件管理流程更新 `.plugin` 文件。两种软件分别确认版本和授权。
